...
- `*.rds` is an extended TSS GRanges object that is passed to great's downstream functions. This is generated by `extendTSS`
- `*.csv` are the individual results tables from GREAT. The pattern should match the format of `<peakset>_<library>.csv`
- `*great.xlsx` is an excel file containing the results from each peak set.
- `great_index.json` is a file that provides the metadata information that was attached to each file.
