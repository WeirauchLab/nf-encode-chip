...
def main(args):
    ...
    if args.encode_bigwig:
        ...
        for file in args.encode_bigwig:
            supertrack.add_track(
                BigWigTrack(bigDataUrl=file, visibility="full", color="0,0,0")
            )
        hub.add_track(supertrack)
    ...
    if args.overlap_peaks:
        ...
        for file in args.overlap_peaks:
            supertrack.add_track(
                BigBedTrack(bigDataUrl=file, visibility="dense", color="0,0,0")
            )
        hub.add_track(supertrack)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--encode_bigwig", type=str, nargs="+", default=[])
    parser.add_argument("--idr_peaks", type=str, nargs="+", default=[])
    parser.add_argument("--overlap_peaks", type=str, nargs="+", default=[])
    parser.add_argument(
